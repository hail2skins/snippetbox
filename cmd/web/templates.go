...
// Define a templateData type to act as the holding structure for any dymamic data
// that we want to pass to our HTML templates. At the moment it only contains one
// field, but we'll add more to it as the build progresses.
...
func newTemplateCache() (map[string]*template.Template, error) {
	...
	for _, page := range pages {
		...
		// This means we have to use template.New() to create an empty template set, use the Funcs() method to register the template.FuncMap,
		// and then parse the files into the template set using the ParseFiles() method.
